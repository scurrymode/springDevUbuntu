...
///home/sist/springDev/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/SpringMongoDBProject/ 
	public void boardGraph(int no){
		try {
			...
			rc.voidEval("data6<-head(sort(data5,decreasing=T),10)");
			rc.voidEval("bp<-barplot(data6,col=rainbow(10),cex.names=0.7,las=2,ylim=c(0,25))");
			rc.voidEval("pct<-round(data6/sum(data6)*100,1)");
			rc.voidEval("text(x=bp,y=data6*1.05,labels=paste(\"(\",pct,\"%\",\")\"),col=\"black\",cex=0.7)");
			rc.voidEval("text(x=bp,y=data6*0.95,labels=paste(data6,\"건\"),col=\"black\",cex=0.7)");
			rc.voidEval("dev.off()");
			rc.close();
		} catch (Exception e) {
			e.printStackTrace();
